...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Streams graph progress in a SINGLE pass and yields:
      ("updates", {node_name: partial_state})  -- for progress display
      ("final", full_state)                    -- once, at the end

    Uses stream_mode=["updates", "values"] so we get both node-by-node
    progress AND correctly-reduced full-state snapshots (values mode applies
    each channel's reducer, e.g. operator.add for `sections`) in one run.
    The final state is just the LAST "values" chunk — the graph is only
    executed once. (An earlier version of this helper called graph_app.invoke()
    again after streaming to get the final state, which silently re-ran the
    entire graph — doubling LLM calls/cost/time and risking a different
    result than what was shown streaming.)
    """
